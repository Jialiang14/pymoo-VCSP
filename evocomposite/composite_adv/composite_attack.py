...
class CompositeAttack(nn.Module):
    ...
    def __init__(self, model, enabled_attack, mode='eval', local_rank=-1,
        ...
        # print(sorted(enabled_attack))
        # print(tuple(sorted(enabled_attack)))
        self.mode = mode
        ...
        eps_pool_number = self.cal_number(enabled_attack)

        self.eps_pool = torch.tensor(
            [(-pi / eps_pool_number[0], pi / eps_pool_number[0]), (0.7/ eps_pool_number[1], 1.3 / eps_pool_number[1]),
             (-10 / eps_pool_number[2], 10 / eps_pool_number[2]),
             (-0.2 / eps_pool_number[3], 0.2 / eps_pool_number[3]),
             (0.7/ eps_pool_number[4], 1.3 / eps_pool_number[4]),
             (-8 / 255 / eps_pool_number[5], 8 / 255 / eps_pool_number[5])])
        # self.eps_pool = torch.tensor([(-pi/eps_pool_number[0], pi/eps_pool_number[0]), (0.7/eps_pool_number[1], 1.3/eps_pool_number[1]), (-10/eps_pool_number[2], 10/eps_pool_number[2]),
        #                               (-0.2/eps_pool_number[3], 0.2/eps_pool_number[3]), (0.7/eps_pool_number[4], 1.3/eps_pool_number[4]), (-8 / 255/eps_pool_number[5], 8 / 255/eps_pool_number[5])])
        # self.eps_pool = torch.tensor(
        #     [(-pi, pi), (0/2, 1/2), (-10, 10 ), (-0.2, 0.2), (0/2, 1/2), (-8 / 255, 8 / 255)])
    ...
    def cal_number(self, enabled_attack):
        enabled_attack = list(enabled_attack)
        eps_pool_number = np.zeros(6)
        eps_pool_number[0] = enabled_attack.count(0)
        eps_pool_number[1] = enabled_attack.count(1)
        eps_pool_number[2] = enabled_attack.count(2)
        eps_pool_number[3] = enabled_attack.count(3)
        eps_pool_number[4] = enabled_attack.count(4)
        eps_pool_number[5] = enabled_attack.count(5)
        return eps_pool_number
    ...
    def eval_hue(self, data, hue, labels):
        ...
        for i in range(self.inner_iter_num):
            ...
            hue = torch.clamp(hue + torch.sign(hue_grad) * self.step_size_pool[0].cuda(), self.eps_pool[0][0].cuda(),
                              self.eps_pool[0][1].cuda()).detach().requires_grad_()
            # print(hue)
            # hue = 0*hue
            # hue = 3.1416 * hue
            new_data = kornia.enhance.adjust_hue(data, hue)
    ...
    def eval_sat(self, data, sat, labels):
        ...
        for i in range(self.inner_iter_num):
            ...
            sat = torch.clamp(sat + torch.sign(sat_grad) * self.step_size_pool[1].cuda(), self.eps_pool[1][0].cuda(),
                              self.eps_pool[1][1].cuda()).detach().requires_grad_()
            new_data = kornia.enhance.adjust_saturation(data, sat)
    ...
    def eval_rot(self, data, theta, labels):
        ...
        for i in range(self.inner_iter_num):
            ...
            theta = torch.clamp(theta + torch.sign(theta_grad) * self.step_size_pool[2].cuda(), self.eps_pool[2][0].cuda(),
                                self.eps_pool[2][1].cuda()).detach().requires_grad_()
            new_data = kornia.geometry.transform.rotate(data, theta)
    ...
    def eval_bright(self, data, brightness, labels):
        ...
        for i in range(self.inner_iter_num):
            ...
            brightness = torch.clamp(brightness + torch.sign(brightness_grad) * self.step_size_pool[3].cuda(),
                                     self.eps_pool[3][0].cuda(),
                                     self.eps_pool[3][1].cuda()).detach().requires_grad_()
            new_data = kornia.enhance.adjust_brightness(data, brightness)
    ...
    def eval_contrast(self, data, contrast, labels):
        ...
        for i in range(self.inner_iter_num):
            ...
            contrast = torch.clamp(contrast + torch.sign(contrast_grad) * self.step_size_pool[4].cuda(), self.eps_pool[4][0].cuda(),
                                   self.eps_pool[4][1].cuda()).detach().requires_grad_()
            new_data = kornia.enhance.adjust_contrast(data, contrast)
    ...
    def eval_linf(self, data, labels):
        ...
        for i in range(self.inner_iter_num):
            ...
            eta = torch.clamp(adv_data.cuda() - ori_data.cuda(), min=self.eps_pool[5][0].cuda(), max=self.eps_pool[5][1].cuda())
            data = torch.clamp(ori_data.cuda() + eta.cuda(), min=0., max=1.).detach_().requires_grad_()
    ...
    def train_contrast(self, data, contrast, labels):
        ...
        for i in range(self.inner_iter_num):
            ...
            if i + 1 == self.inner_iter_num:
                ...
                contrast_grad = torch.autograd.grad(cost, contrast, retain_graph=False)[0]
            # value = self.eps_pool[4][0].cuda()
            # print(value.device)
            contrast = torch.clamp(contrast + torch.sign(contrast_grad) * self.step_size_pool[4], self.eps_pool[4][0].cuda(),
                                   self.eps_pool[4][1].cuda()).detach().requires_grad_()
    ...
    def train_linf(self, data, labels):
        ...
        for i in range(self.inner_iter_num):
            ...
            eta = torch.clamp(adv_data - ori_data, min=self.eps_pool[5][0].cuda(), max=self.eps_pool[5][1].cuda())
    ...
    def attack_eval(self, images, labels):
        ...
        for i in range(self.start_num):
            ...
            for j in range(self.iter_num):
                # self.curr_seq = torch.tensor([0,1,2,3,4,5])
                # print('iter : {} order update --> {}'.format(j, self.curr_seq))
                # print(self.curr_seq )
