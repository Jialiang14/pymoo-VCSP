...
import csv
import random
from AAA.optimizer_attack.evocomposite import utils
...
import numpy as np
from optimizer_adv.SE_RNR_MP_PGDAT.model import NetworkCIFAR as Network
import warnings
...
def main_worker(gpu, ngpus_per_node, args, model):
    ...
    valid_queue = torch.utils.data.DataLoader(
        valid_data, batch_size=16, shuffle=False, pin_memory=False, num_workers=2)
...
def gen_caa_noise(model, input, target, attack_number):
    attacks = []
    if attack_number == 1:
        ...
        attack_names: List[str] = [
            "CompositeAttack(model, enabled_attack=(0,), mode='train', order_schedule='fixed', inner_iter_num=1)"
        ]
    elif attack_number == 3:
        attack_names: List[str] = [
            "CompositeAttack(model, enabled_attack=(1,), mode='train', order_schedule='fixed', inner_iter_num=1)"
        ]
    elif attack_number == 4:
        attack_names: List[str] = [
            "CompositeAttack(model, enabled_attack=(2,), mode='train', order_schedule='fixed', inner_iter_num=1)"
        ]
    elif attack_number == 5:
        attack_names: List[str] = [
            "CompositeAttack(model, enabled_attack=(3,),  mode='train', order_schedule='fixed', inner_iter_num=1)"
        ]
    elif attack_number == 6:
        attack_names: List[str] = [
            "CompositeAttack(model, enabled_attack=(4,),  mode='train', order_schedule='fixed', inner_iter_num=1)"
        ]
    elif attack_number == 7:
        attack_names: List[str] = [
            "CompositeAttack(model, enabled_attack=(0,1,2,3,4,5),  mode='train', order_schedule='fixed', inner_iter_num=1)"
        ]
    ...
    # print(input.shape)
    # original = input[0, :, :, :]
    # images_original = original.squeeze(0)
    # print(images_original.shape)
    # plt.axis('off')
    # plt.imshow(images_original.permute(1, 2, 0).cpu().detach().numpy())
    # plt.savefig('/mnt/jfs/sunjialiang/AAAD/retrain/MultiRobustness_self/original.png', bbox_inches='tight', pad_inches=0.02)
    #
    # ae = adv_inputs[0,:,:,:]
    # images = ae.squeeze(0)
    # print(images.shape)
    ...
    # plt.savefig('/mnt/jfs/sunjialiang/AAAD/retrain/MultiRobustness_self/5.png', bbox_inches='tight', pad_inches=0.02)
    return adv_inputs

def evaluate_MP(model, val_loader):
    attacks = []
    attack_names: List[str] = [
        "NoAttack()" ,
    ...
    "CompositeAttack(model, enabled_attack=(4,), order_schedule='fixed', inner_iter_num=10)",
        "CompositeAttack(model, enabled_attack=(0,1,2,3,4,5), order_schedule='fixed', inner_iter_num=10)"
        # "CompositeAttack(model, enabled_attack=(0,1,5), order_schedule='fixed', inner_iter_num=10)",
        # "CompositeAttack(model, enabled_attack=(0,1,2,5), order_schedule='fixed', inner_iter_num=10)",
        # "CompositeAttack(model, enabled_attack=(0,1,2,3,5), order_schedule='fixed', inner_iter_num=10)"
    ]

    # "CompositeAttack(model, enabled_attack=(5,), order_schedule='fixed', inner_iter_num=1)"]
    for attack_name in attack_names:
        # print(attack_name)
        tmp = eval(attack_name)
        attacks.append(tmp)
