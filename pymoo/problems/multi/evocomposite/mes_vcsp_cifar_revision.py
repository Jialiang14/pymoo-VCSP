...
parser.add_argument('--max_gen', type=int, default=1,
...
class NSGASearch():
    ...
    def calculate_fitness(self, pop):
        ...
        for i in range(0, self.popsize):
            print(pop[i][:])
            Robust_accuracy, l2_distance, time_cost = main_worker(self.args, self.model, pop[i][:])
            function1.append(Robust_accuracy)
            function2.append(l2_distance)
            function3.append(time_cost)
    ...
    def parento_plot(self, args, Parent_pop, function1_values2, function2_values2, function3_values2, gen_no):
        plt.clf()
        ...
        # plt.show()
        save_path = '/mnt/jfs/sunjialiang/AAAD/AAA/optimizer_attack/evocomposite/Initial1'
        # if not os.path.exists(save_path):
        #     os.mkdir(save_path)
        plt.savefig('/mnt/jfs/sunjialiang/AAAD/AAA/optimizer_attack/evocomposite/Initial1/'+ 'gen_' + str(gen_no) + '_caa.jpg')
        ...
        plt.clf()
        font_merge = self.fast_non_dominated_sort(function1_values2, function3_values2)
        f12 = [function1_values2[k] for k in font_merge[0]]
        f22 = [function3_values2[k] for k in font_merge[0]]
        firstpareto = [Parent_pop[k] for k in font_merge[0]]
        # f22 = [item.item() for item in f22]
        index = []
        for i in range(len(f22)):
            index.append(f22.index(sorted(f22)[i]))
        for i in range(len(f22) - 1):
            j = index[i]
            k = index[i + 1]
            plt.plot([f22[j], f22[k]], [f12[j], f12[k]], color='r')
        plt.xlabel('Time cost', fontsize=15)
        plt.ylabel('Robust accuracy', fontsize=15)
        # function3_values2 = [item.cpu().detach().item() for item in function3_values2]
        plt.scatter(function3_values2, function1_values2, c='blue')
        # plt.show()
        save_path = '/mnt/jfs/sunjialiang/AAAD/AAA/optimizer_attack/evocomposite/Initial2'
        # if not os.path.exists(save_path):
        #     os.mkdir(save_path)
        plt.savefig(
            '/mnt/jfs/sunjialiang/AAAD/AAA/optimizer_attack/evocomposite/Initial2/' + 'gen_' + str(gen_no) + '_caa.jpg')
        scio.savemat(save_path + '/nsga' + str(gen_no) + '.mat', {'function1_values': f1,
                                                                  'function2_values': f2})
        scio.savemat(save_path + '/first_pareto_' + str(gen_no) + '.mat', {'first_pareto': firstpareto})

        plt.clf()
        font_merge = self.fast_non_dominated_sort(function2_values2, function3_values2)
        f13 = [function2_values2[k] for k in font_merge[0]]
        f23 = [function3_values2[k] for k in font_merge[0]]
        firstpareto = [Parent_pop[k] for k in font_merge[0]]
        # f23 = [item.cpu().detach().item() for item in f23]
        index = []
        for i in range(len(f13)):
            index.append(f13.index(sorted(f13)[i]))
        # for i in range(len(f13) - 1):
        #     j = index[i]
        #     k = index[i + 1]
        #     plt.plot([f13[j], f13[k]], [f23[j], f23[k]], color='r')
        plt.xlabel('L2', fontsize=15)
        plt.ylabel('Time cost', fontsize=15)
        # function3_values2 = [item.cpu().detach().item() for item in function3_values2]
        plt.scatter(function2_values2, function3_values2, c='blue')
        # plt.show()
        save_path = '/mnt/jfs/sunjialiang/AAAD/AAA/optimizer_attack/evocomposite/Initial3'
        # if not os.path.exists(save_path):
        #     os.mkdir(save_path)
        plt.savefig(
            '/mnt/jfs/sunjialiang/AAAD/AAA/optimizer_attack/evocomposite/Initial3/' + 'gen_' + str(gen_no) + '_caa.jpg')
        scio.savemat(save_path + '/nsga' + str(gen_no) + '.mat', {'function1_values': f1,
                                                                  'function2_values': f2})
        scio.savemat(save_path + '/first_pareto_' + str(gen_no) + '.mat', {'first_pareto': firstpareto})
    ...
    def evolve(self):
        ...
        function1_parent, function2_parent, function3_parent = self.calculate_fitness(Parent_pop)
        print('Initialization succeed.')
        gen_no = 0
        while (gen_no < self.args.max_gen):
            print('gen:', gen_no)
            print(Parent_pop)
            self.parento_plot(self.args, Parent_pop, function1_parent, function2_parent, function3_parent, gen_no)
            # 产生子代个体
...
def main_worker(args, model, x):
    ...
    fitness, l2_distance, time_cost = evaluate(model, test_loader, attack_names, attacks, args)
    return fitness, l2_distance, time_cost
...
def evaluate(model, val_loader, attack_names, attacks, args):
    ...
    # return batch_accuracy * 100,  Dist_norm/(32*21)+time_cost Initial1
